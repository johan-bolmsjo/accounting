...
func (table *Table) RenderText() []byte {
	...
	if table.titles != nil {
		fmt.Fprintf(&buf, "%s", table.titles.renderText(columnWidths))
	...
	for _, row := range table.rows {
		fmt.Fprintf(&buf, "%s", row.renderText(columnWidths))
	}
	return buf.Bytes()
}
...
func (table *Table) String() string {
	return string(table.RenderText())
}

// Render row as text.
func (row *Row) renderText(columnWidths []int) []byte {
	var buf bytes.Buffer
	...
	for i := 0; i < len(columnWidths); i++ {
		...
		fmt.Fprintf(&buf, "%s%s", begPad, row.cellAt(i).renderText(columnWidths[i]))
	}
